...
def password_genr():
    ...
    data_pass = {webs: {"email":email,"password":password}}

    try:
        with open(file="password.json",mode="r") as file_j:
            x=json.load(file_j)
            x.update(data_pass)
            print(x)
    except FileNotFoundError:
        with open(file="password.json",mode="w") as file_x:
            json.dump(data_pass,file_x)
            r
    with open(file="password.json",mode="w") as file_w:
        json.dump(x,file_w,indent=4)
